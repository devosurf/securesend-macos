...
# Builds the thing people download: SecureSend.app signed with a real Developer
# ID, stapled, inside a signed and stapled dmg. Stapling both is what makes the
...
STAGE="$OUT/stage"
APP="$STAGE/SecureSend.app"
DMG="$OUT/SecureSend-$VERSION.dmg"
...
# ~/Applications, so what ships is what you have been running. REGISTER=false
# keeps a copy that is about to be deleted out of Launch Services.
export SIGN_IDENTITY
DEST="$STAGE" REGISTER=false "$ROOT/scripts/build.sh"
...
if [ "$NOTARIZING" = true ]; then
	...
	ditto -c -k --keepParent "$APP" "$OUT/SecureSend.zip"
	notarize "$OUT/SecureSend.zip"
	xcrun stapler staple "$APP"
	rm -f "$OUT/SecureSend.zip"
fi
